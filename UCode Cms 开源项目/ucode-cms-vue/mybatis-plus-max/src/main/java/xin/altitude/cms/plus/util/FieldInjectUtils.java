...
public class FieldInjectUtils {
     ...
     */
    @SafeVarargs
    public static <V extends FkField<R>, S, R extends Serializable, SS extends IService<S>> void injectField(V data, Class<SS> serviceClazz, SFunction<V, ? extends Serializable>... injectColumns) {
        IService<S> iService = SpringUtils.getBean(serviceClazz);
        // 获取主表外键值
        R id = data.fkVal();
        // 如果集合元素个数不为空 则进行后续操作
        if (id != null && injectColumns.length > 0) {
            List<String> injectFiledNames = RefUtils.getFiledNames(injectColumns);
            String[] selectField = {String.join(",", injectFiledNames)};
            // 副表主键查询数据
            S s = iService.getBaseMapper().selectById(id);
            // 使用Spring内置的属性复制工具类
            BeanCopyUtils.copyProperties(s, data, selectField);
        }
    }
     ...
     */
    @SafeVarargs
    public static <V, S, R> void injectField(V data, final SFunction<V, R> fkColumn, Class<? extends IService<S>> clazz, final SFunction<S, R> pkColumn, SFunction<V, ? extends Serializable>... injectColumns) {
        IService<S> iService = SpringUtils.getBean(clazz);
        // 获取主表关联外键的集合（去重）
        R id = EntityUtils.toObj(data, fkColumn);
        // 如果集合元素个数不为空 则进行后续操作
        if (id != null && injectColumns.length > 0) {
            // 主表关联外键对应的字段名字符串
            ...
            // 构造副表查询条件(查询指定列元素)
            QueryWrapper<S> wrapper = Wrappers.query(RefUtils.newInstance(iService.getEntityClass()))
                .select(selectStr).eq(GuavaUtils.toUnderScoreCase(fieldName), id);
            // 通过主表的外键查询关联副表符合条件的数据
            S one = iService.getBaseMapper().selectOne(wrapper);
            // 使用Spring内置的属性复制方法
            BeanCopyUtils.copyProperties(one, data, selectField);
        }
    }
     ...
     */
    @SafeVarargs
    public static <V extends FkField<R>, S, R extends Serializable> void injectField(List<V> data, Class<? extends IService<S>> clazz, SFunction<V, ? extends Serializable>... injectColumns) {
        IService<S> iService = SpringUtils.getBean(clazz);
        // 获取主表关联外键的集合（去重）
        Set<R> ids = EntityUtils.toSet(data, (SFunction<V, R>) FkField::fkVal);
        // 如果集合元素个数不为空 则进行后续操作
        if (ids.size() > 0 && injectColumns.length > 0) {
            List<String> injectFiledNames = RefUtils.getFiledNames(injectColumns);
            String[] selectField = {String.join(",", injectFiledNames)};
            // 通过主表的外键查询关联副表符合条件的数据
            List<S> list = iService.getBaseMapper().selectBatchIds(ids);
            Map<R, S> map = new HashMap<>();
            for (S s : list) {
                map.put(PlusUtils.pkVal(s, iService.getEntityClass()), s);
            }
            doInjectField(data, map, RefUtils.getFiledNames(injectColumns));
        }
    }
     ...
     */
    @SafeVarargs
    public static <V extends FkField, S, R> void injectField(IPage<V> page, Class<? extends IService<S>> clazz, SFunction<V, ? extends Serializable>... injectColumns) {
        List<V> records = page.getRecords();
        injectField(records, clazz, injectColumns);
    }
     ...
     * @param <R>        主表关联外键、副表主键的类型泛型
     */
    private static <T extends FkField<R>, S, R extends Serializable> void doInjectField(List<T> data, Map<R, S> map, List<String> filedNames) {
